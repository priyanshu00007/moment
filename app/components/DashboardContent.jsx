...
import {
  CircleCheck, Clock, ListTodo, Star, Loader2, AlertCircle,
  RefreshCw, Target, Timer, Calendar, BarChart3, Trash2
} from 'lucide-react';
...
// Dashboard data aggregation and stats class
class DashboardDataManager {
  ...
  static getDailyStats() {
    ...
    focusData.forEach(session => {
      ...
      if (!dailyStats[date]) {
        dailyStats[date] = {
          date, focusTime: 0, focusSessions: 0,
          pomodoroTime: 0, pomodoroSessions: 0,
          tasksCreated: 0, tasksCompleted: 0,
          tasksStarted: 0, totalActiveTime: 0
        };
      }
      if (session.duration) {
        dailyStats[date].focusTime += Math.floor(session.duration / 60);
        dailyStats[date].totalActiveTime += Math.floor(session.duration / 60);
      }
      if (session.completed !== false) dailyStats[date].focusSessions += 1;
    });
    ...
    pomodoroData.forEach(session => {
      ...
      if (session.duration) {
        dailyStats[date].pomodoroTime += Math.floor(session.duration / 60);
        dailyStats[date].totalActiveTime += Math.floor(session.duration / 60);
      }
      if (session.completed !== false && session.type === 'work') dailyStats[date].pomodoroSessions += 1;
    });
  ...
  static getWeeklyData() {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayStats = dailyStats[dateStr] || {
        date: dateStr, focusTime: 0, focusSessions: 0,
        pomodoroTime: 0, pomodoroSessions: 0,
        tasksCreated: 0, tasksCompleted: 0,
        tasksStarted: 0, totalActiveTime: 0
      };
  ...
  static getTotalStats() {
    const dailyStats = this.getDailyStats();
    const totals = {
      totalFocusTime: 0, totalPomodoroTime: 0,
      totalFocusSessions: 0, totalPomodoroSessions: 0,
      totalTasksCreated: 0, totalTasksCompleted: 0,
      totalActiveTime: 0, averageDailyFocus: 0,
  ...
  static getSessionEfficiency() {
    ...
    const pomodoroData = this.getPomodoroData();
    const efficiency = {
      focusCompletionRate: 0, pomodoroCompletionRate: 0,
      averageFocusSession: 0, averagePomodoroSession: 0
    };
    ...
    efficiency.averageFocusSession = focusSessions > 0
      ? Math.round(totalFocusDuration / focusSessions / 60) : 0;
    ...
    efficiency.averagePomodoroSession = pomodoroSessions > 0
      ? Math.round(totalPomodoroDuration / pomodoroSessions / 60) : 0;
    return efficiency;
  }
}

// Button to clear profile data with confirmation
function ClearProfileButton({ onClear }) {
    ...
    <div className="mt-8">
      {!confirming ? (
        <button
          ...
          aria-label="Clear all profile data and start over"
        >
          <Trash2 className="w-4 h-4" />
          <span>Clear Profile & Start Over</span>
        </button>
      ) : (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 mt-2 flex flex-col items-center">
          <p className="text-red-700 font-semibold mb-2">
            Are you sure? This will delete all your profile and analytics data. This action cannot be undone.
          </p>
...
const StatCard = ({ title, value, subtitle, icon: Icon, color, trend }) => (
  <div className="bg-white p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100 flex flex-col h-full justify-between">
...
const StatCardSkeleton = () => (
  <div className="bg-gray-200 p-6 rounded-2xl animate-pulse h-full flex flex-col"></div>
);
...
export default function DashboardContent() {
  ...
  const loadDashboardData = () => {
    try {
      ...
      setDashboardStats({
        ...totalStats, weeklyData, priorityDistribution, sessionEfficiency
      });
    } catch (err) {
  ...
  const fetchTasks = async () => {
    try {
      ...
      setTasks([]); setUserData(defaultUserData);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    refreshInterval.current = setInterval(() => loadDashboardData(), 30000);
  ...
  // Clear profile & data from localStorage and reload app
  function handleClearProfile() {
    localStorage.removeItem('user_stats');
    localStorage.removeItem('focus-sessions-data');
    localStorage.removeItem('pomodoro-sessions-data');
    localStorage.removeItem('tasks-analytics-data');
    localStorage.removeItem('user-activities-data');
  ...
  const stats = dashboardStats || {
    totalFocusTime: 0, totalPomodoroTime: 0,
    totalFocusSessions: 0, totalPomodoroSessions: 0,
    totalTasksCompleted: 0, weeklyData: [],
    ...
    <div className="p-4 sm:p-6 lg:p-8 bg-gray-50 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
        ...
        <div className="mt-4 md:mt-0">
          ...
          <ClearProfileButton onClear={handleClearProfile} />
        </div>
      </div>

      {/* Main stats cards */}
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch mb-8">
        <StatCard
      ...
      {/* Additional stats */}
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch mb-8">
        <StatCard
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
        ...
        {/* Recent Activity */}
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-lg hover:shadow-xl transition-shadow flex flex-col h-full">
          <div className="flex items-center justify-between mb-6">
...
// woking without cleaning 
// 'use client';
// import React, { useState, useEffect, useRef } from 'react';
// import { useUser } from '@clerk/nextjs';
// import { 
//   CircleCheck, Clock, ListTodo, TrendingUp, Star, 
//   Loader2, AlertCircle, RefreshCw, Target, Activity, Timer, 
//   Calendar, BarChart3 
// } from 'lucide-react';
// import { format, subDays, isToday } from 'date-fns';
// import { 
...
//         setTasks(backendTasks);
//         const generatedUserData = generateUserDataFromTasks(
//           backendTasks, "user123", user?.firstName || user?.fullName || "User"
//         );
//         setUserData(generatedUserData); 
//       } else {
//         throw new Error(response.error || 'API request failed');
