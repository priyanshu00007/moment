...
import { Plus, Search, Loader2, Bell, Settings, X, ChevronDown } from "lucide-react";
...
// Productivity Level Modal Component
const ProductivityModal = ({ isOpen, onSubmit, currentLevel }) => {
  const [selectedLevel, setSelectedLevel] = useState(currentLevel || 'medium');
      ...
      {isOpen && (
        <>
          <motion.div
            ...
            onClick={(e) => e.target === e.currentTarget && onSubmit(selectedLevel)}
          >
            <motion.div
            ...
            >
              <div className="text-center mb-6">
                ...
                <p className="text-gray-600 text-sm">
                  This helps us recommend the right tasks for you
                </p>
              </div>
              ...
              <div className="mb-6">
                <div className="relative">
                  <select
                    ...
                    className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl appearance-none text-gray-800 font-medium cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  >
                    <option value="low">Low - Taking it easy today</option>
                    <option value="medium">Medium - Steady and focused</option>
                    <option value="high">High - Feeling very productive</option>
                  </select>
...
const NotificationPanel = ({ isOpen, onClose, notifications, onClearAll, onMarkRead }) => {
  if (!isOpen) return null;
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="fixed right-4 top-20 w-80 bg-white rounded-xl shadow-2xl border border-gray-200 z-50 max-h-96 overflow-hidden"
      >
...
export default function TasksPage() {
  ...
  const [notifications, setNotifications] = useState([
    {
      id: 1,
      title: "Task Reminder",
      message: "You have 3 pending high-priority tasks",
      timestamp: new Date(),
      read: false,
      type: "reminder"
    },
    {
      ...
      message: "Great job! You've completed 5 tasks today",
  ...
  // Add notification when tasks are due or completed
  useEffect(() => {
    ...
    if (dueTasks.length > 0) {
      const newNotification = {
        ...
        message: `${dueTasks.length} task${dueTasks.length > 1 ? 's' : ''} due today or tomorrow`,
        timestamp: new Date(),
        read: false,
        type: "due"
      };
      ...
      setNotifications(prev => {
        ...
        if (prev.some(n => n.type === "due" && n.message === newNotification.message)) {
          return prev;
        }
  ...
  const handleSetProductivity = (level) => {
    ...
    const newNotification = {
      id: Date.now(),
      title: "Productivity Level Set",
      message: `Your productivity level is now set to ${level}`,
      timestamp: new Date(),
      read: false,
      type: "setting"
    };
    ...
    toast.success(`Productivity level set to ${level}`);
  };
  ...
  const handleSelectMode = async (mode, task) => {
    try {
      ...
      const newNotification = {
        id: Date.now(),
        title: `${mode === 'focus' ? 'Focus' : 'Pomodoro'} Session Started`,
  ...
  // Filter tasks based on productivity level and other filters
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              ...
              <span className="text-sm font-medium">
                {productivityLevel ? `${productivityLevel.charAt(0).toUpperCase() + productivityLevel.slice(1)} Mode` : 'Set Productivity'}
              </span>
            </button>
          </div>
        </div>
        ...
        {productivityLevel && (
          <div className="mb-6 p-4 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl border border-indigo-100">
            <h3 className="text-lg font-semibold text-gray-800 mb-2">
              ...
              <span className={`ml-2 capitalize px-3 py-1 rounded-full text-sm ${
                productivityLevel === 'high' ? 'bg-green-100 text-green-700' :
                productivityLevel === 'medium' ? 'bg-yellow-100 text-yellow-700' :
            ...
            <p className="text-sm text-gray-600">
              {productivityLevel === 'high' 
                ? "🚀 You're feeling productive! High-priority tasks are shown first."
        ...
        <div className="mb-8">
          <h2 className="text-xl font-semibold text-gray-800 mb-4 flex items-center gap-2">
            <span>Active Tasks ({recommendedTasks.length})</span>
            {productivityLevel && (
          ...
          {recommendedTasks.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-xl border border-gray-200">
              ...
              <p className="text-gray-500 text-lg mb-4">No active tasks found</p>
...
// import { motion, AnimatePresence } from "framer-motion";
// import { Plus, Search, Loader2, Bell, Settings, X, ChevronDown, Filter, TrendingUp, Clock, Target, Calendar } from "lucide-react";
// import { useRouter } from 'next/navigation';
// import TaskCard from "@/app/components/TaskCard";
// import TaskModal from "@/app/components/TaskModal";
// import StartTaskModal from "@/app/components/StartTaskModal";
...
//   storageKey: 'tasks-analytics-data',
  
//   saveTaskActivity(activity) {
//     try {
...
//       const activities = JSON.parse(localStorage.getItem(this.storageKey) || '[]');
...
//         tasksStarted: todayActivities.filter(a => a.type === 'session_started').length,
//         totalFocusTime: todayActivities
//           .filter(a => a.type === 'session_completed' && a.sessionType === 'focus')
//           .reduce((acc, a) => acc + (a.duration || 0), 0),
//         totalPomodoroTime: todayActivities
//           .filter(a => a.type === 'session_completed' && a.sessionType === 'pomodoro')
//           .reduce((acc, a) => acc + (a.duration || 0), 0),
//         productivityChanges: todayActivities.filter(a => a.type === 'productivity_changed').length
//       };
//     } catch (error) {
//       console.error('Error getting today stats:', error);
//       return { tasksCreated: 0, tasksCompleted: 0, tasksStarted: 0, totalFocusTime: 0, totalPomodoroTime: 0, productivityChanges: 0 };
//     }
//   },
...
//     try {
//       const activities = JSON.parse(localStorage.getItem(this.storageKey) || '[]');
//       const taskCreations = activities.filter(a => a.type === 'task_created');
      
//       return {
//         high: taskCreations.filter(a => a.priority === 'high').length,
//         medium: taskCreations.filter(a => a.priority === 'medium').length,
//         low: taskCreations.filter(a => a.priority === 'low').length
//       };
//     } catch (error) {
...
//                 </h2>
//                 <p className="text-gray-600 text-sm">
//                   This helps us show only the right tasks for you
//                 </p>
//               </div>
...
// // Enhanced Notification Component with analytics
// const NotificationPanel = ({ isOpen, onClose, notifications, onClearAll, onMarkRead }) => {
//   if (!isOpen) return null;
...
// // Daily Analytics Dashboard Component
// const DailyAnalytics = ({ todayStats, weeklyTrends, productivityLevel }) => {
...
//           <div className="text-2xl font-bold text-blue-600">{todayStats.tasksCreated}</div>
//           <div className="text-sm text-blue-700 font-medium">Created</div>
//           <div className="text-xs text-blue-600">New Tasks</div>
//         </div>
...
//           </div>
//           <div className="text-sm text-purple-700 font-medium">Focus Time</div>
//           <div className="text-xs text-purple-600">Deep Work</div>
//         </div>
        
//         <div className="text-center p-3 bg-orange-50 rounded-lg">
//           <div className="text-2xl font-bold text-orange-600">
//             {formatDuration(todayStats.totalPomodoroTime)}
//           </div>
//           <div className="text-sm text-orange-700 font-medium">Pomodoro</div>
...
//                 style={{ 
//                   height: `${Math.max((day.tasksCompleted / Math.max(...weeklyTrends.map(d => d.tasksCompleted), 1)) * 40, 4)}px`,
//                   backgroundColor: day.date === new Date().toISOString().split('T')[0] ? '#4f46e5' : '#c7d2fe'
//                 }}
//               />
//               <div className="text-xs text-gray-500 mt-1">{day.day}</div>
//               <div className="text-xs font-medium text-gray-700">{day.tasksCompleted}</div>
//             </div>
//           ))}
//         </div>
//       </div>
...
//   // Analytics State
//   const [todayStats, setTodayStats] = useState(TaskDataManager.getTodayStats());
//   const [weeklyTrends, setWeeklyTrends] = useState(TaskDataManager.getWeeklyTrends());
...
//       setTodayStats(TaskDataManager.getTodayStats());
//       setWeeklyTrends(TaskDataManager.getWeeklyTrends());
//     }, 30000);
...
//   // Enhanced notification system with due date tracking
//   useEffect(() => {
//     if (!productivityLevel) return;
    
//     const relevantTasks = tasks.filter(task => task.priority === productivityLevel);
//     const dueTasks = relevantTasks.filter(task => {
//       if (!task.dueDate || task.status === 'completed') return false;
//       const dueDate = new Date(task.dueDate);
//       const today = new Date();
//       const timeDiff = dueDate.getTime() - today.getTime();
...
//       const newNotification = {
//         id: Date.now(),
//         title: `${productivityLevel.charAt(0).toUpperCase() + productivityLevel.slice(1)} Priority Tasks Due`,
//         message: `${dueTasks.length} ${productivityLevel}-priority task${dueTasks.length > 1 ? 's' : ''} due soon`,
//         timestamp: new Date(),
//         read: false,
//         type: "due"
//       };
...
//     // Record productivity change
//     TaskDataManager.saveTaskActivity({
//       type: 'productivity_changed',
//       fromLevel: productivityLevel,
//       toLevel: level,
//       userId: "user123"
//     });
...
//     const newNotification = {
//       id: Date.now(),
//       title: "Productivity Level Updated",
//       message: `Now showing only ${level}-priority tasks`,
//       timestamp: new Date(),
//       read: false,
//       type: "setting"
//     };
...
//     toast.success(`Now showing ${level}-priority tasks only`);
//   };
...
//         // Add notification only if task matches current productivity level
//         if (taskData.priority === productivityLevel) {
//           const newNotification = {
//             id: Date.now(),
//             title: "New Task Added",
//             message: `"${taskData.title}" matches your current productivity level`,
//             timestamp: new Date(),
//             read: false,
//             type: "task_created"
//           };
...
//               Math.floor((new Date() - new Date(task.createdAt)) / 1000) : null,
//             userId: "user123"
//           });
...
//             id: Date.now(),
//             title: "Task Completed! 🎉",
//             message: `Great job completing "${task.title}" (${task.priority} priority)`,
//             timestamp: new Date(),
//             read: false,
//             type: "completion"
//           };
...
//         id: Date.now(),
//         title: `${mode === 'focus' ? 'Focus' : 'Pomodoro'} Session Started`,
//         message: `Working on "${task.title}" (${task.priority} priority)`,
//         timestamp: new Date(),
//         read: false,
//         type: "session"
//       };
...
//           taskTitle: selectedTask.title,
//           priority: selectedTask.priority,
//           completedViaSession: activeSession,
//           userId: "user123"
//         });
...
//   // Filter tasks - ONLY show tasks matching the productivity level
//   const productivityFilteredTasks = productivityLevel 
//     ? tasks.filter(task => task.priority === productivityLevel)
//     : [];

//   // Apply additional filters to productivity-filtered tasks
//   const filteredTasks = productivityFilteredTasks.filter(task => {
//     if (filters.status !== "all" && task.status !== filters.status) return false;
//     if (filters.category !== "all" && task.category !== filters.category) return false;
//     if (filters.search && !task.title.toLowerCase().includes(filters.search.toLowerCase())) return false;
//     return true;
...
//   // Count all tasks by priority for statistics
...
//           <div className="text-6xl mb-4">⚡</div>
//           <h2 className="text-2xl font-bold text-gray-800 mb-4">Set Your Productivity Level</h2>
//           <p className="text-gray-600 mb-6">Choose your current productivity level to see relevant tasks</p>
//           <button
//             onClick={() => setShowProductivityModal(true)}
//             className="bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 transition-colors"
//           >
...
//               My {productivityLevel.charAt(0).toUpperCase() + productivityLevel.slice(1)} Priority Tasks
//             </h1>
//             <p className="text-gray-600">
//               Showing only {productivityLevel}-priority tasks based on your productivity level
//             </p>
//           </div>
...
//               <Settings className="w-4 h-4" />
//               <span className="text-sm font-medium">
//                 {productivityLevel.charAt(0).toUpperCase() + productivityLevel.slice(1)} Mode
//               </span>
//             </button>
//           </div>
//         </div>
...
//                   'bg-blue-100 text-blue-700'
//                 }`}>
//                   {productivityLevel} Priority
//                 </span>
//               </h3>
//               <p className="text-sm text-gray-600">
//                 {productivityLevel === 'high' 
//                   ? "🚀 Showing high-priority tasks only. You're in peak productivity mode!"
//                   : productivityLevel === 'medium'
//                   ? "⚡ Showing medium-priority tasks only. Perfect for steady progress!"
//                   : "🌱 Showing low-priority tasks only. Great for building momentum!"}
//               </p>
//             </div>
...
//               <div className="text-center">
//                 <div className="font-semibold text-green-700">{allTasksByPriority.high}</div>
//                 <div>High</div>
//               </div>
//               <div className="text-center">
//                 <div className="font-semibold text-yellow-700">{allTasksByPriority.medium}</div>
//                 <div>Medium</div>
//               </div>
//               <div className="text-center">
//                 <div className="font-semibold text-blue-700">{allTasksByPriority.low}</div>
//                 <div>Low</div>
//               </div>
//             </div>
//           </div>
...
//               <input
//                 type="text"
//                 placeholder={`Search ${productivityLevel}-priority tasks...`}
//                 value={filters.search}
//                 onChange={(e) => setFilters({ ...filters, search: e.target.value })}
//                 className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
//               />
...
//             >
//               <Plus className="w-5 h-5" />
//               Add {productivityLevel.charAt(0).toUpperCase() + productivityLevel.slice(1)} Priority Task
//             </button>
//           </div>
//         </div>
...
//         <div className="mb-8">
//           <h2 className="text-xl font-semibold text-gray-800 mb-4 flex items-center gap-2">
//             <span>Active {productivityLevel.charAt(0).toUpperCase() + productivityLevel.slice(1)} Priority Tasks ({activeTasks.length})</span>
...
//                 {productivityLevel === 'high' ? '🚀' : productivityLevel === 'medium' ? '⚡' : '🌱'}
//               </div>
//               <p className="text-gray-500 text-lg mb-4">
//                 No {productivityLevel}-priority tasks found
//               </p>
//               <p className="text-gray-400 mb-6">
//                 {filteredTasks.length === 0 
//                   ? `Create your first ${productivityLevel}-priority task to get started!`
//                   : `All your ${productivityLevel}-priority tasks are completed!`
//                 }
//               </p>
//               <button
//                 onClick={() => {
...
//                 className="bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 transition-colors"
//               >
//                 Create {productivityLevel.charAt(0).toUpperCase() + productivityLevel.slice(1)} Priority Task
//               </button>
//             </div>
//           ) : (
//             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
...
//                     onDelete={handleDeleteTask}
//                     onToggleComplete={handleToggleComplete}
//                     isRecommended={true} // All tasks shown are recommended based on productivity level
//                   />
//                 ))}
//               </AnimatePresence>
//             </div>
...
//           <div>
//             <h2 className="text-xl font-semibold text-gray-800 mb-4">
//               Completed {productivityLevel.charAt(0).toUpperCase() + productivityLevel.slice(1)} Priority Tasks ({completedTasks.length}) ✅
//             </h2>
//             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
//               <AnimatePresence>
//                 {completedTasks.map((task, idx) => (
...
//         }
//         task={editingTask}
//         defaultPriority={productivityLevel} // Set default priority to current productivity level
//       />
...
// // import React, { useState, useEffect } from "react";
// // import { motion, AnimatePresence } from "framer-motion";
// // import { Plus, Search, Loader2, Bell, Settings, X, ChevronDown, Filter } from "lucide-react";
// // import { useRouter } from 'next/navigation';
// // import TaskCard from "@/app/components/TaskCard";
// // import TaskModal from "@/app/components/TaskModal";
// // import StartTaskModal from "@/app/components/StartTaskModal";
